...
Visit http://localhost:3000 and start chatting! Open a project across two browser tabs to see everything getting updated in realtime 🥳
...
Users with the `moderator` role can delete all layers. Users with the `admin` role can delete all layers and projects (note: it's not recommended to delete the `public` project).
...
-- Custom types
create type public.app_permission as enum ('projects.delete', 'layers.delete');
...
comment on table public.projects is 'Topics and groups.';
...
comment on table public.layers is 'Individual layers sent by each user.';
...
create policy "Allow individual insert access" on public.projects for insert with check ( auth.uid() = created_by );
create policy "Allow individual delete access" on public.projects for delete using ( auth.uid() = created_by );
create policy "Allow authorized delete access" on public.projects for delete using ( authorize('projects.delete', auth.uid()) );
create policy "Allow logged-in read access" on public.layers for select using ( auth.role() = 'authenticated' );
create policy "Allow individual insert access" on public.layers for insert with check ( auth.uid() = user_id );
create policy "Allow individual update access" on public.layers for update using ( auth.uid() = user_id );
create policy "Allow individual delete access" on public.layers for delete using ( auth.uid() = user_id );
create policy "Allow authorized delete access" on public.layers for delete using ( authorize('layers.delete', auth.uid()) );
...
insert into public.projects (slug, created_by)
values
    ('public', '8d0fd2b3-9ca7-4d9e-a95f-9e13dded323e'),
    ('random', '8d0fd2b3-9ca7-4d9e-a95f-9e13dded323e');

insert into public.layers (layer, project_id, user_id)
values
