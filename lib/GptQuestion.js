...
async function askGPT (prompt, openai, type) {
  ...
  try {
    ...
    try {
      ...
      localforage.setItem(prompt, { result: data, request: result.data, prompt })
...
export async function GptQuestion ({
  openai, node: {
    name, each, objective, context, pattern, type, question, children, loop
  }, variables = {}
}) {
  if (loop) {
    const array = []
  ...
  const newPrompt = ` ${objective}
                      ${context}
                      ${question} ${type ? `in the form of ${type}` : ''} ${pattern ? `following the pattern: ${pattern}` : ''}`
