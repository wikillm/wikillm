...
class FinderRow extends Component {
  ...
  onChange() {
    const { rowIndex, columnIndex, disable, hasChildren, onChange, ...value } =
      this.props;
    if (!disable && onChange) {
...
class Finder extends Component {
  constructor(props:any) {
    ...
    const { columns, newSelectIndexes } = this.parseData(
      valueIndexes || selectIndexes || defaultSelectIndexes,
      data,
      dataKeys
    );
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      !equal(this.props.data, data) ||
      (valueIndexes && valueIndexes !== this.state.selectIndexes)
    ) {
      const { columns, newSelectIndexes } = this.parseData(
        valueIndexes || selectIndexes || defaultSelectIndexes,
        data,
  ...
  getValueIndexes(value, data, dataKeys) {
    ...
    const valueIndexes = dataPathByValue[value]
      ? dataPathByValue[value].indexes
      : [];
    return valueIndexes;
  }
  ...
  getTreePath(data, dataKeys = { children: 'children', value: 'value' }) {
    ...
    function lookAll(data, dataKeys, indexes) {
      ...
      for (const key in data) {
        const item = data[key];
        const newIndexes = JSON.parse(JSON.stringify(indexes)); // TODO: remove this
        newIndexes.push(key);
        item.indexes = newIndexes;
        if (
          item[dataKeys.children] &&
          Object.keys(item[dataKeys.children]).length > 0
        ) {
          item.children = lookAll(
            item[dataKeys.children],
            dataKeys,
            newIndexes
          );
  ...
  parseData(selectIndexes = [], data = [], dataKeys = {}) {
    ...
    do {
      ...
      dataItem =
        ...
        dataItem[selectIndex][dataKeys.children];
      i++;
    } while (dataItem);
